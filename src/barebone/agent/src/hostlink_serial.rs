...
fn issue<F>(request: F) -> Option<usize>
where
    F: FnOnce(*mut c_void, *mut c_void) -> i32,
{
    ...
    unsafe {
        if (_ZwCreateEvent)(&mut event, EVENT_ALL_ACCESS, ptr::null_mut(), NOTIFICATION_EVENT, 0)
                < 0 {
            return None;
        }
    }
...
fn read_exactly(port: *mut c_void, buffer: &mut [u8]) -> bool {
    ...
    while read != buffer.len() {
        let mut offset = 0i64;
        let moved = issue(|event, status_block| unsafe {
            (_ZwReadFile)(
                port,
                event,
                ptr::null_mut(),
        ...
        if moved == 0 {
            ...
            kernel::wait(&raw const IDLE_TOKEN as *const u8, Some(IDLE_PAUSE_US), &mut || false);
            continue;
        }
        read += moved;
    }
...
fn control_port(port: *mut c_void, code: u32, input: *const u8, length: u32) -> bool {
    issue(|event, status_block| unsafe {
...
static IDLE_TOKEN: u8 = 0;
const IDLE_PAUSE_US: u64 = 2000;
